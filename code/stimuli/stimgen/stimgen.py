...
def CreateWordSet(path_out='../../../data/letters/',
    ...
    for word in wordlist:
        ...
        ed = int(n_imgs*0.8) # 80% train data
        IXs_train = IXs[:ed] 
        IXs_test = IXs[ed:]

        n_train_imgs = len(IXs_train)
        ed = int(n_train_imgs*0.2) # %20 val data (from train)
        IXs_val = IXs_train[:ed]
        IXs_train = IXs_train[ed:]
        
        for IX_train in IXs_train:
            path = os.path.join(path_out, 'train', metas[IX_train]['word'])
            os.makedirs(path, exist_ok=True)
            fn = dict2string(metas[IX_train])
            imgs[IX_train].save(os.path.join(path, fn))
